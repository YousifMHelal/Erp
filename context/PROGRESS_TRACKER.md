...
| 10 | Post-launch additions | DONE | P10-1 backup & restore, P10-2 backup reminder schedule shipped. |

**Overall: 106 / 106 v1 tasks done, plus 2 post-launch additions (P10-1, P10-2).**
...
| ID | Task | Status | Notes |
|----|------|--------|-------|
| P10-1 | Backup & restore, `/settings/backup` | DONE | `lib/backup.ts` (model registry + wipe/read/write helpers, FK-safe order), `GET /api/backup/export` (streamed JSON download), `actions/backup.actions.ts#restoreBackup` (password re-check + full wipe/reload in one transaction). Gated by a dedicated `settings.backup` permission (added after initial ship — see MEMORY.md). |
| P10-2 | Backup reminder schedule | DONE | `lib/backup-reminder.ts` (`isBackupReminderDue`, pure UTC+2 schedule math), `actions/backup.actions.ts#getBackupReminder/saveBackupReminder/syncBackupReminder`. Reminder card in `BackupPanel`; fires as a `SYSTEM` notification via the existing dedupe-key pattern, checked opportunistically on every dashboard load — no cron. |
...
### 2026-09-25 (Reports PDF export in the shop print template)
- **Report print/PDF redesigned** for every `/reports/*` page (all render through `ReportShell`). A print-only `#print-root.report-print.invoice-a4` document replaces the old tiny header: grey title bar (report title + shop name), info block (period, print date/time shop-local — refreshed on `beforeprint` — active filter names, the print template's shop-level fields) with the template logo, a bordered totals-summary row built from the report's footer, a fixed-size print chart (700×240px, grey bars, `maxBarSize` cap), and an invoice-style table (row numbers, `#f2f2f2` head repeating per page, bold grey totals row, amounts via `formatAmount` without ج.م). Shop info/logo load server-side in `lib/report-print.ts#loadReportPrintShop` and pass to the client shell as plain strings. A4 page gets `10mm 8mm` margins via an inline `@page` (same pattern as the statement print); print CSS now flattens the app-shell ancestors of a report print root. Screen chart also caps bar width so a single period no longer renders as one solid block.

### 2026-09-24 (Phase 10 — Backup, restore & reminder schedule)
- **P10-2 complete.** Backup reminder schedule added to `/settings/backup`. `lib/backup-reminder.ts#isBackupReminderDue` is a pure function computing whether the configured schedule's most recent occurrence (daily/weekly/monthly, shop-local UTC+2 time) has passed without yet being fired, reusing the locked `SHOP_UTC_OFFSET_HOURS` convention. No cron/background process exists in this app, so `syncBackupReminder()` runs opportunistically inside `getDashboardOverview()` on every dashboard load — a due reminder fires the next time anyone visits after the scheduled moment passes, not at the exact second. When due, it creates a dedupe-keyed (`backup-reminder:<date>`) `SYSTEM` notification (reusing the existing `Notification` table/bell UI, same pattern as low-stock/balance alerts) and stamps `Setting["backup.reminderLastFiredAt"]` so it won't re-fire for the same occurrence. No file is ever generated or sent automatically — this is a reminder only.
  - **Permission change mid-build:** user asked for backup/restore to have its own permission instead of reusing `settings.manage`. Added `settings.backup` to `lib/permissions.ts`'s `settings` group, re-gated the export route, `restoreBackup`, `getBackupReminder`, and `saveBackupReminder` on it, and excluded it from the seeded محاسب role (mirroring its existing `settings.manage` exclusion). The live dev DB's admin role was already seeded before this change, so it had no `settings.backup` grant — fixed with a targeted one-row `UPDATE` (script written, run, then deleted) rather than a full `db:reset`, since the user explicitly chose the non-destructive option when Prisma's dangerous-migration guard blocked an unconfirmed `migrate reset --force`.
  - Verified live end-to-end via Playwright: saved a weekly schedule, reloaded the page, confirmed it persisted; then set a daily 00:00 schedule (guaranteed already due), loaded the dashboard, and confirmed via direct DB query that exactly one notification was created and `lastFiredAt` was stamped — then reloaded the dashboard again and confirmed no duplicate notification and no `lastFiredAt` change (dedup holds). `typecheck`/`lint` clean throughout.
- **P10-1 complete.** New `/settings/backup` tab. Export (`GET /api/backup/export`) streams a single JSON file with every one of the 20 Prisma models. Restore (`restoreBackup` Server Action) re-verifies the acting user's password via bcrypt, requires a typed Arabic confirmation phrase in the UI, then wipes every table and reloads the uploaded file's contents inside one `$transaction({ timeout: 60_000 })` — full wipe + reload, not a partial/merge restore, since the FK graph across invoices/ledgers/parties makes a merge unsafe. `lib/backup.ts` holds `BACKUP_MODELS` in FK-safe order (parents-before-children for restore, reversed for wipe) plus `readAllTables`/`wipeAllTables`/`writeAllTables`. This is the third route-handler exception to "Server Actions for everything" (alongside NextAuth and the invoice PDF stream) — justified the same way: a file download needs to stream a `Response` body, which a Server Action can't do. Found and fixed a real bug during manual verification: the export's `$transaction` hit Prisma's 5s default interactive-transaction timeout against Neon's real latency across 20 tables (same class of bug already logged for collections/payments) — fixed by making the read-only export run as plain queries instead of an interactive transaction, since a snapshot read has no cross-table invariant to protect. Full round-trip verified live (export → restore the same file → `npm run db:reconcile` passed for all cashboxes/customers/suppliers) — zero data corruption. `typecheck`/`lint` clean.
